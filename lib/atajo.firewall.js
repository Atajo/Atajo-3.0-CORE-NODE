...
class Firewall {
    ...
    start() {
        ...
        this.insights.trackEvent({name: "atajo.core.firewall.start"});
        ...
        new Consul()
            ...
            .subscribe(response => {
                ...
                this.insights.trackEvent({name: "atajo.core.firewall.consul.error", properties: { error: error}});
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                try {
                    ...
                    this.insights.trackEvent({name: "atajo.core.firewall.reject", properties: { domain: domain, error: e.stack}});
                ...
                if (!acl) {
                    ...
                    this.insights.trackEvent({name: "atajo.core.firewall.reject", properties: { domain: domain, error: "FIREWALL: INVALID DOMAIN FOR " + key + " @ " + domain}});
                    socket.emit("core:message", {
                        disconnect: true,
                        message: "INVALID DOMAIN / SECRET"
                    });
                ...
                if (domain && (matchKey == key)) {
                    ...
                    if (isProvider) {
                        ...
                        io
                            ...
                            .clients((err, clients) => {
                                ...
                                if (clients.length == 0) {
                                    ...
                                    this.insights.trackEvent({name: "atajo.core.firewall.lambda.connect", properties: { domain : socket.identity.domain, uuid :  socket.identity.uuid, ip:  socket.identity.ip}});
                        ...
                        if(socket.identity.device) { 
                            ...
                            for(let i in socket.identity.device) {   
                                trackingObj[i] = ( (!!socket.identity.device[i]) && (socket.identity.device[i].constructor === Object) ) ? JSON.stringify(socket.identity.device[i]) : socket.identity.device[i]
                            }

                        }

                        this.insights.trackEvent({name: "atajo.core.firewall.client.connect", properties: trackingObj });
                    ...
                    this.insights.trackEvent({name: "atajo.core.firewall.reject", properties: {domain: domain, key:key }});
                ...
                this.insights.trackEvent({name: "atajo.core.firewall.error", properties: {error: e}});
        ...
        if (identity.provider) {
            ...
            io
                ...
                .clients((err, clients) => {
                    ...
                    if (clients.length == 0) {
                        ...
                        this.insights.trackEvent({name: "atajo.core.firewall.lambda.disconnect", properties: trackingObj });
            ...
            if(identity.device) { 
                for(let i in identity.device) {   
                    trackingObj[i] = ( (!!identity.device[i]) && (identity.device[i].constructor === Object) ) ? JSON.stringify(identity.device[i]) : identity.device[i]
                }
            }


            this.insights.trackEvent({name: "atajo.core.firewall.client.disconnect", properties: trackingObj});
