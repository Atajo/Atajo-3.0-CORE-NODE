...
class Firewall {
    ...
    clear(socket) {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                let uuid = (isProvider) ? query.hostname.toLowerCase() : query.uuid.toLowerCase();
                let key = (isProvider) ? query.secret.toLowerCase() : query.key.toLowerCase();
                let domain = query.domain.toLowerCase();

                log.debug("DOMAIN IS : ", domain)
                let device = (isProvider) ? {} : query.device;
                let acl = this.providers[domain];

                try {
                    if (!isProvider) { device = JSON.parse(device); }
                } catch (e) {
                    log.warn("FIREWALL: DEVICE DID NOT IDENTIFY ITSELF PROPERLY -> REJECTING");
                    socket.emit("core:message", { disconnect: true, message: "INVALID DOMAIN / SECRET" });
                    return;
                }
                ...
                let matchKey = (isProvider) ? acl.secret : acl.key;

                let update = { uuid: uuid, key: key, domain: domain, ip: remoteIpAddress };

                if (domain && (matchKey == key)) {

                    log.debug("VALID " + (isProvider ? "PROVIDER" : "DEVICE") + " CONNECTED WITH " + key + " @ " + domain);
                    update.valid = true;
                    update.state = "CONNECTED";

                    if (isProvider) { new dbi.providerConnectionLogs(update).save() } else { new dbi.clientConnectionLogs(update).save() };
                    ...
                    if (isProvider) {
                        socket.identity = { pid: guid(), uuid: uuid, domain: domain, key: key, ip: remoteIpAddress, provider: true, device: device };


                        io.in(domain + '_provider').clients((err, clients) => {

                            if (clients.length == 0) {
                                //FIRST CONNECTION. FAIL BUSY
                                log.debug("FIRST CONNECT: SETTING ALL BUSY TRANSACTIONS FOR DOMAIN " + domain + " TO FAILED");
                                dbi.transactions.update({ domain: domain, status: 'BUSY' }, { $set: { status: 'FAILED' } }).then(err => {
                            ...
                            dbi.providerConnectionStates.findOneAndUpdate({ domain: domain }, update, { upsert: true }, (err, obj) => {});
                            io.to(domain).emit('domain:connect', { nodes: clients.length });
                        });

                    } else {
                        socket.identity = { pid: guid(), uuid: uuid, domain: domain, key: key, ip: remoteIpAddress, provider: false, device: device };
                        socket.join(domain);

                        log.debug("UPDATING CLIENT CONNECTION STATES UUID " + uuid + " WITH ", update);

                        dbi.clientConnectionStates.findOneAndUpdate({ uuid: uuid }, update, { upsert: true }, (err, obj) => {});
                        that.getProviderNode(domain, function(id) {

                            io.to(id).emit('client:connect', socket.identity);
        ...
        if (identity.provider) {
            ...
            io.in(domain + '_provider').clients((err, clients) => {
                ...
                if (clients.length == 0) {
                    ...
                    new dbi.providerConnectionLogs({
                    ...
                    dbi.transactions.update({ domain: identity.domain, status: 'BUSY' }, { $set: { status: 'FAILED' } }).then(err => {
                ...
                let connected = (clients.length == 0) ? false : true;
                let update = { nodes: clients.length, connected: connected }
                io.to(domain).emit('domain:disconnect', { nodes: clients.length });
                dbi.providerConnectionStates.findOneAndUpdate({ domain: identity.domain }, update, { upsert: true }, (err, obj) => {});
                this.triggerHook('lambda', 'disconnect', { domain: domain, identity: identity, status: update });
            ...
            that.getProviderNode(domain, function(id) {
                ...
                io.to(id).emit('client:disconnect', identity);
    ...
    getProviderNode(domain, cb) {
        ...
        io.in(domain + '_provider').clients((err, clients) => {
            ...
            if (clients.length == 0) { cb(false); return; }
    ...
    getClientId(uuid, domain, cb) {
        ...
        dbi.clientConnectionStates.findOne({ uuid: uuid, domain: domain }, { id: 1 }).then(client => {
